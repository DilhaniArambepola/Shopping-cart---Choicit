...
// if user clicked register button
    ...
    if (isset($_POST['btnSave'])) {
        ...
        $sql = "INSERT INTO `accessaries` (`Accessary_ID`,`Accessary_name`, `Selling_price`, `Shipping_cost`, `prodQty`, `Prod_img`,`Prod_Description`) "
    ...
    if (isset($_POST['btnUpdate'])) {
        // register the user
        //  php source code to EXECUTE INSERT INTO Query
        $sql = "UPDATE `accessaries` SET `Accessary_ID`='$pid', `Accessary_name`='$prodName', `Selling_price`='$price', `Shipping_cost`='$shipping', `prodQty`='$qty', `Prod_Description`= '$description' WHERE `Accessary_ID`='$pid';";
    }
    

    if ($conn->query($sql) === TRUE) { // successfully added
        header("location: ../add-Accessaries.php?success");
    } else {
