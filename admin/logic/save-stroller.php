...
// if user clicked register button
    ...
    $target_dir = "../../images/Babies/strollers/";
    ...
    if (isset($_POST['btnSave'])) {
        ...
        $sql = "INSERT INTO `strollers` (`stroller_ID`,`stroller_name`, `Selling_price`, `Shipping_cost`, `prodQty`, `Prod_img`,`Prod_Description`) "
    ...
    if (isset($_POST['btnUpdate'])) {
        // register the user
        //  php source code to EXECUTE INSERT INTO Query
        $sql = "UPDATE `strollers` SET `stroller_ID`='$pid', `stroller_name`='$prodName', `Selling_price`='$price', `Shipping_cost`='$shipping', `prodQty`='$qty', `Prod_Description`= '$description' WHERE `stroller_ID`='$pid';";
    }
    

    if ($conn->query($sql) === TRUE) { // successfully added
        header("location: ../add-stroller.php?success");
    } else {
