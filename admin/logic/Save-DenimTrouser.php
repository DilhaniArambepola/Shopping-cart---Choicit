...
// if user clicked register button
    ...
    if (isset($_POST['btnSave'])) {
        ...
        $sql = "INSERT INTO `trouser` (`Denim_ID`,`Denim_name`, `Selling_price`, `Shipping_cost`, `prodQty`, `Prod_img`,`Prod_Description`) "
    ...
    if (isset($_POST['btnUpdate'])) {
        // register the user
        //  php source code to EXECUTE INSERT INTO Query
        $sql = "UPDATE `trouser` SET `Denim_ID`='$pid', `Denim_name`='$prodName', `Selling_price`='$price', `Shipping_cost`='$shipping', `prodQty`='$qty', `Prod_Description`= '$description' WHERE `Denim_ID`='$pid';";
    }
    

    if ($conn->query($sql) === TRUE) { // successfully added
        header("location: ../add-DenimTrouser.php?success");
    } else {
