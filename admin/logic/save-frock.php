...
// if user clicked register button
    ...
    if (isset($_POST['btnSave'])) {
        ...
        $sql = "INSERT INTO `frocks` (`Frock_ID`,`Frock_name`, `Selling_price`, `Shipping_cost`, `prodQty`, `Prod_img`,`Prod_Description`) "
    ...
    if (isset($_POST['btnUpdate'])) {
        // register the user
        //  php source code to EXECUTE INSERT INTO Query
        $sql = "UPDATE `frocks` SET `Frock_ID`='$pid', `Frock_name`='$prodName', `Selling_price`='$price', `Shipping_cost`='$shipping', `prodQty`='$qty', `Prod_Description`= '$description' WHERE `Frock_ID`='$pid';";
    }
    

    if ($conn->query($sql) === TRUE) { // successfully added
        header("location: ../add-Frocks.php?success");
    } else {
