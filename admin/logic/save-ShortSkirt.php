...
// if user clicked register button
    ...
    if (isset($_POST['btnSave'])) {
        ...
        $sql = "INSERT INTO `short_skirt` (`SSkirt_ID`,`SSkirt_name`, `Selling_price`, `Shipping_cost`, `prodQty`, `Prod_img`,`Prod_Description`) "
    ...
    if (isset($_POST['btnUpdate'])) {
        // register the user
        //  php source code to EXECUTE INSERT INTO Query
        $sql = "UPDATE `short_skirt` SET `SSkirt_ID`='$pid', `SSkirt_name`='$prodName', `Selling_price`='$price', `Shipping_cost`='$shipping', `prodQty`='$qty', `Prod_Description`= '$description' WHERE `SSkirt_ID`='$pid';";
    }
    

    if ($conn->query($sql) === TRUE) { // successfully added
        header("location: ../add-ShortSkirt.php?success");
    } else {
