...
// if user clicked register button
    ...
    if (isset($_POST['btnSave'])) {
        ...
        $sql = "INSERT INTO `long_skirt` (`LSkirt_ID`,`LSkirt_name`, `Selling_price`, `Shipping_cost`, `prodQty`, `Prod_img`,`Prod_Description`) "
    ...
    if (isset($_POST['btnUpdate'])) {
        // register the user
        //  php source code to EXECUTE INSERT INTO Query
        $sql = "UPDATE `long_skirt` SET `LSkirt_ID`='$pid', `LSkirt_name`='$prodName', `Selling_price`='$price', `Shipping_cost`='$shipping', `prodQty`='$qty', `Prod_Description`= '$description' WHERE `LSkirt_ID`='$pid';";
    }
    

    if ($conn->query($sql) === TRUE) { // successfully added
        header("location: ../add-LongSkirt.php?success");
    } else {
